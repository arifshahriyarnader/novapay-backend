...
export const internationalTransfer = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    const userId = req.user!.userId;
    const input = req.body as InternationalTransferInput;
    const result = await internationalTransferService(userId, input);
    return apiResponse(res, 201, "International transfer successful", result);
  },
);
